    ...
    <Screen title="PassPath Premium" subtitle="Everything you need to actually pass." onBack={() => router.back()}>
      ...
      {status?.isPremium ? (
        ...
        <Card style={{ alignItems: 'center', backgroundColor: colors.brand }}>
          ...
          <Text style={{ color: colors.white, fontSize: 40, fontFamily: 'Poppins_700Bold', marginTop: 4 }}>{status?.priceLabel ?? 'R150/month'}</Text>
      ...
      {status?.isPremium ? (
        ...
        >
          ...
          <Text style={{ color: colors.white, fontSize: 16, fontFamily: 'Poppins_700Bold' }}>{busy ? 'Opening secure checkout…' : `Subscribe — ${status?.priceLabel ?? 'R150/month'}`}</Text>
        </Pressable>
      )}
